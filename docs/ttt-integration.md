...
Authenticated endpoints (`/futures/assets`, `…/positions`, `…/orders`, `…/trades`): require a key. AsA treats the combined `trade-futures` scope as highly sensitive and DOES NOT consume these routes by default (`NOT CONFIGURED` unless TTT_API_KEY/SECRET are set, for read use only, isolated from the live pipeline). Trading endpoints are never implemented (`/futures/orders` writes do not exist anywhere in the codebase — verified by `tests/no-fake-scan.test.ts`).
